...
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i+1}});next=imagesCollection[index]||imagesCollection[0];$(".lightboxImage").attr("src",$(next).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
        lightboxId ? lightboxId : "galleryLightbox"
            ...
            </div>`)},showItemTags(gallery,position,tags){var tagItems='<li class="nav-item" role="menuitem"><button class="nav-link active active-tag"  data-images-toggle="all">Tous</button></li>';$.each(tags,function(index,value){tagItems+=`<li class="nav-item active" role="menuitem">
                <button class="nav-link"  data-images-toggle="${value}">${value}</button></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills" role="menubar">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){if($(this).hasClass("active-tag")){return}
